...
/**
 * <UK>
 * Shibboleth authentication customized for Chalres University Digital Repository
 * 
 * First, method isFromCU gathers information about the IP address of the user trying to authenticate via Shibboleth,
 * if user is connecting from within the range of allowed IP address, it will store this information in a boolean memberCU
 * When boolean is true, it will take this into account when special group is assigned to the authenticated user and another special group is then
 ...
 * @author <a href="https://knihovna.cuni.cz">Jakub Řihák</a>
 * @version $Revision$
 */
public class UKShibAuthentication implements AuthenticationMethod
{
	/** log4j category */
	// setting a custom log4j category for customized UKShibAuthentication method
	private static Logger log = Logger.getLogger(UKShibAuthentication.class);
	...
	/**
	* <UK>
	* IP-checking part of the Shibboleth Authentication Process
	*
	*/
	...
	/** Maps IPMatchers to groupnames gathered from config file */
	private Map<IPMatcher, String> ipMatcherConfigGroupNames;
	/**
	* </UK>
	*/
    ...
    public int authenticate(Context context, String username, String password,
		...
		// <JR> 8. 2. 2021
		// Five steps to authenticate a user
		try {

			// Step 2: Identify User
			EPerson eperson = findEPerson(context, request);

			// Step 3: Register New User, if necessary
    ...
    public List<Group> getSpecialGroups(Context context, HttpServletRequest request)
	{
		...
		try {
			...
			log.debug("UK IP Shibboleth Authentication: Starting to determine special groups");
			...
			if (affiliations == null) {
				...
				log.debug("UK IP Shibboleth Authentication: Failed to find Shibboleth role header, '"+roleHeader+"', falling back to the default roles: '"+ StringUtils.join(defaultRoles, ",") + "'");
			} else {
				log.debug("UK IP Shibboleth Authentication: Found Shibboleth role header: '"+roleHeader+"' = '"+affiliations+"'");
			}
			...
			try {
				...
				if (allGroups != null) {
					for ( Group group: allGroups ) {
						...
						// <JR> If a mapping between DSpace group and Shibboleth attribute(s) exists in authentication-shibboleth.cfg for one of the DSpace groups, this will run
						...
						for ( String attribute: groupAttributes ) {
							...
							String expectationStr = configurationService.getProperty("authentication-shibboleth.group." + group.getName() + "." + attribute);
							// <JR> We are trying to find the attribute in the list send through Shibboleth
							...
							log.debug("<JR> Attribute: '"+attribute+"' search '" + expectationStr +"' in '" + valueStr + "' found '" + result + "'");
						}

						if ( member ) {
							// <JR> - We found a attribute and its value (or one of its values) is matching the pattern in authentication-shibboleth.cfg
							// We can add special group to the user
							//
							// But, before we add it, we'll check, if user is sending request from within the allowed IP ranges
							// If this is true:
							//
							// We'll check if a special group for IP-allowed-shibboleth-authenticated users exists - if yes, we'll add this group to the user
							...
							if(checkAllowedIP == true)
							{
								if (isFromCU(context, request) == true)
								{
									...
									log.debug("Trying to find IP allowed group...");
									ipAllowedGroup = findIpAllowedGroup(context, groupPrefix + group.getName());
									...
									if (ipAllowedGroup == null) {
										...
										log.info("Mapping role affiliation to DSpace IP-allowed group: " + ipAllowedGroup.getID() + " with name: " + ipAllowedGroup.getName());
										groups.add(ipAllowedGroup);
									}
								
								}
								else {
									log.info("UK IP Shibboleth Authentication: Request is not comming from within CU!");
									log.info("Mapping role affiliation to regular DSpace group: " + group.getID() + " with name: " + group.getName());
									groups.add(group);
								}	
							}
	...
	{	
		...
		if ( lazySession ) {
			...
			if (!(port == 443 || port == 80))
				returnURL += ":" + port;
			 // <JR> - 8. 2. 2021 - Fixing redirects - DSpace 6 version
            // - DSpace 5 version available at: https://github.com/UKUK-Repository-Dept/ukuk-dspace/commit/64ae19678abb57184e2f6f1d81e5f44336c95957
		...
		if (netidHeader != null) {
			...
			if (netid != null) {
				...
				if (eperson == null)
					...
					log.info("Identified EPerson based upon Shibboleth netid header: '"+netidHeader+"'='"+netid+"'.");
			}
		}
	...
	/**
	* Method that tries to find a group based on its name within a given context.
	* @param context
	* 			current context
	* @param groupName
	* 			name of the group that should be looked-up
	* @return found group or null if no group was found 
	*/
	//
	// TODO: This should be general method to find a identified by name in a given context
	protected Group findIpAllowedGroup(Context context, String groupName) {
		Group group = null; // <JR> 3. 3. 2021
		try {
			log.info("Searching for IP allowed group with name: "+groupName);
			log.debug("findIpAllowedGroup - trimmed grouName: " + groupName.trim());
			group = groupService.findByName(context,groupName.trim());
					
			if (group != null){
				log.info("Groupname is="+groupName+" / GroupID is="+group.getID());
			}
			else 
			{
				log.info("Unable to find group: '"+groupName.trim()+"'");
			}
		}
		catch (SQLException sqle) {
			log.error("Exception thrown while trying to find group: '"+groupName.trim()+"'",sqle);
		}
	...
	* If matching group is defined in authentication-ip.cfg as well, IP matchers are created for all IP ranges defined for that group
	* in authentication-ip.cfg file.
	*
	* @param context
	*			current context
	*
	...
	*/
	private void createIPMatchers(Context context) throws IPMatcherException
	{
		...
		if (shibUniGroupName != null)
		{
			...
			List<String> ipAuthProperties = configurationService.getPropertyKeys("authentication-shibboleth-ip");
			log.debug("<JR> - ipAuthProperties: " + ipAuthProperties);
			for(String propName : ipAuthProperties)
			{
				if (propName.startsWith("authentication-shibboleth-ip.allowedGroupName"))
				{
					...
					{	
						String ipConfigGroupName = nameParts[2];
						...
						if (ipConfigGroupName.equals(shibUniGroupName))
						{
							...
							addIPMatchers(ipConfigGroupName, configurationService.getArrayProperty("authentication-shibboleth-ip.allowedGroupName." + ipConfigGroupName));
						}
						else
						{
							// otherwise log error
	...
	// groupName is gathered from the authentication-ip config file - it is a simple string
	// this gets called for every group if IP (or IP ranges) in the authentication-ip config
